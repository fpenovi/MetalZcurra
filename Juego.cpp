...
void handleEvent( SDL_Event& e, Cliente &cliente )
{
	...
	if( e.type == SDL_KEYDOWN && e.key.repeat == 0 )
	{
		...
		{
			case SDLK_LEFT:
				string left = to_string(SDLK_LEFT);
				cliente.enviarAusuario("TODOS", left, false);
				//velx -= Personaje_VEL;derecha = false;break;
			case SDLK_RIGHT:
				string right = to_string(SDLK_RIGHT);
				cliente.enviarAusuario("TODOS", right, false);
				//velx += Personaje_VEL;derecha = true;break;
			case SDLK_UP:
				string up = to_string(SDLK_UP);
				cliente.enviarAusuario("TODOS", up, false);
